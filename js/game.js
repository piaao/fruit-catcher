...
function getGravityStrength() {
  const lvl = LEVELS[level];
  return lvl.gravityStrength || 0;
}

function initGravityLevel() {
  const str = getGravityStrength();
  if (str === 0) {
    gravityState.active = false;
    return;
  }
  gravityState.active = true;
  gravityState.strength = str;
  gravityState.mode = Math.random() < 0.5 ? 'attract' : 'repel';
  gravityState.fieldAngle = 0;
  if (typeof playGravitySound === 'function') playGravitySound(gravityState.mode, str);
}
...
function renderGravityField() {
  ...
  const modeIcon = gravityState.mode === 'attract' ? '⚡引力' : '💨斥力';
...
function getGemStrength() {
  const lvl = LEVELS[level];
  return lvl.gemStrength || 0;
}

function initGemLevel() {
  gemState.active = getGemStrength() > 0;
  gemState.chains = [];
  gemState.strength = getGemStrength();
}
...
function updateGemBoosts(dt) {
  projectiles.forEach(p => {
    if (p.gemBoosted) {
      p.gemBoostTimer -= dt;
      if (p.gemBoostTimer <= 0) {
        p.gemBoosted = false;
      }
    }
  });
...
function drawGemIndicators() {
  ...
  projectiles.forEach(p => {
    ...
    ctx.save();
    ctx.globalAlpha = 0.4 + Math.sin(performance.now() / 180) * 0.2;
    ctx.strokeStyle = '#ffd700';
    ctx.lineWidth = 2;
    ctx.shadowColor = '#ffd700';
    ctx.shadowBlur = 12;
    ctx.beginPath();
    ctx.arc(p.x, p.y, (p.def.radius + 6) * pulseScale, 0, Math.PI * 2);
    ctx.stroke();

    for (let i = 0; i < 3; i++) {
      const angle = (performance.now() / 400 + i * (Math.PI * 2 / 3)) % (Math.PI * 2);
      const dist = p.def.radius + 10;
      ctx.globalAlpha = 0.6 + Math.sin(performance.now() / 150 + i) * 0.3;
...
function initGravityGemDemo() {
  ...
  let t = 0;
  let gravityMode = 'attract'; // 吸引模式

  // 水果
  let demoFruits = [];
  function spawnDemoFruit() {
    const side = Math.floor(Math.random() * 4);
    let sx, sy;
    if (side === 0) { sx = Math.random() * W2; sy = -10; }
    else if (side === 1) { sx = W2 + 10; sy = Math.random() * H2; }
    else if (side === 2) { sx = Math.random() * W2; sy = H2 + 10; }
    else { sx = -10; sy = Math.random() * H2; }
    demoFruits.push({
      x: sx, y: sy,
      vx: (CX2 - sx) / Math.hypot(CX2 - sx, CY2 - sy) * 60,
      vy: (CY2 - sy) / Math.hypot(CX2 - sx, CY2 - sy) * 60,
      emoji: ['💎', '🍎', '🍐', '🔴'][Math.floor(Math.random() * 4)],
      isGem: Math.random() < 0.3,
    });
  }
  for (let i = 0; i < 4; i++) spawnDemoFruit();

  // 宝石连锁波
  let chains = [];

  // 角色指示器位置
  let playerPos = { x: CX2, y: CY2 };
  ...
  function drawFrame() {
    ...
    ctx.rotate(t * 0.3);
    for (let i = 1; i <= 4; i++) {
      const r = (i / 4) * Math.min(W2, H2) * 0.4;
      ctx.beginPath();
      ctx.arc(0, 0, r, 0, Math.PI * 2);
      ctx.strokeStyle = 'rgba(255,215,0,' + (0.05 - i * 0.01) + ')';
      ctx.lineWidth = 1;
      ctx.stroke();
    }
    ctx.restore();
    ...
    demoFruits.forEach(f => {
      if (!f.eaten) {
        ...
        const dy = playerPos.y - f.y;
        const d = Math.hypot(dx, dy);
        if (d > 5) {
          const force = gravityMode === 'attract' ? 80 : -60;
          const nf = force / Math.max(d / 50, 1);
          f.vx += (dx / d) * nf * 0.016;
          f.vy += (dy / d) * nf * 0.016;
        }
        f.x += f.vx * 0.016;
        ...
        if (d < 15) {
          ...
          if (f.isGem) {
            // 创建连锁波
            chains.push({ x: f.x, y: f.y, r: 5, maxR: 60, alpha: 1, color: 'rgba(255,215,0,0.4)' });
            setTimeout(() => {
              chains.push({ x: f.x, y: f.y, r: 5, maxR: 45, alpha: 1, color: 'rgba(255,100,50,0.3)' });
            }, 150);
          }
        }
      }
    ...
    // 清理出界水果
    demoFruits = demoFruits.filter(f => !f.eaten && f.x > -20 && f.x < W2 + 20 && f.y > -20 && f.y < H2 + 20);
    while (demoFruits.length < 4) spawnDemoFruit();

    // 绘制连锁波
    chains.forEach(c => {
      c.r += 120 * 0.016;
      c.alpha = Math.max(0, 1 - c.r / c.maxR);
      ctx.save();
    ...
    chains = chains.filter(c => c.r < c.maxR);

    // 绘制水果
    demoFruits.forEach(f => {
      if (f.isGem) {
        // 金色光圈
        const pulseScale = 1 + Math.sin(t * 5) * 0.15;
        ctx.save();
        ctx.globalAlpha = 0.5;
        ctx.strokeStyle = '#ffd700';
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.arc(f.x, f.y, (9 + 4) * pulseScale, 0, Math.PI * 2);
        ctx.stroke();
        ctx.restore();
      }
      ctx.save();
    ...
    // 说明文字
    ctx.save();
    const modeText = gravityMode === 'attract' ? '⚡引力场吸引水果 💎宝石触发连锁' : '💨斥力场推开水果 💎宝石触发连锁';
    ctx.fillStyle = 'rgba(255,215,0,0.9)';
    ctx.font = 'bold 10px sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(modeText, W2 / 2, H2 - 10);
    ctx.restore();

    // 定期切换重力模式
    if (Math.floor(t) % 5 === 0 && Math.floor(t) > 0) {
      gravityMode = gravityMode === 'attract' ? 'repel' : 'attract';
    }

    gravityGemDemoRAF = requestAnimationFrame(drawFrame);
  }

  if (gravityGemDemoRAF) cancelAnimationFrame(gravityGemDemoRAF);
  gravityGemDemoRAF = requestAnimationFrame(drawFrame);
}

function stopGravityGemDemo() {
  if (gravityGemDemoRAF) {
    cancelAnimationFrame(gravityGemDemoRAF);
    gravityGemDemoRAF = null;
  }
}
...
function showLevelPreview() {
  ...
  // 显示重力场（仁果殿主题）
  const gravityInfo = document.getElementById('previewGravityInfo');
  if (gravityInfo) {
    if (lvl.gravityStrength && lvl.gravityStrength > 0) {
      const gLabels = ['弱引力', '中引力', '强引力'];
      gravityInfo.textContent = '⚡ 重力场：' + gLabels[lvl.gravityStrength - 1];
      gravityInfo.style.display = 'block';
    } else {
      gravityInfo.style.display = 'none';
    }
  }

  // 显示宝石类型（仁果殿主题）
  const gemInfo = document.getElementById('previewGemInfo');
  if (gemInfo) {
    if (lvl.gemStrength && lvl.gemStrength > 0) {
      const gemText = lvl.gemStrength === 1 ? '石榴（3层冲击波）' : '石榴+榴莲果（2层大范围冲击波）';
      gemInfo.textContent = '💎 宝石连锁：' + gemText;
    ...
    projectiles.forEach(p => {
      ...
      if (hitTest(p)) {
        ...
        if (p.isBomb) {
          ...
          // 宝石连锁触发（仁果殿主题 - 在吃水果后立即检测）
          onGemEaten(p);

          // 宝石连锁加分（被冲击波击中的水果得分翻倍）
          if (p.gemBoosted) {
            pts = Math.round(pts * 2);
            addFloatingText(h.x + 20, h.y - 10, '✨x2', '#ffd700');
          }
