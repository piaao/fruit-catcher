...
/** 重力场启动音效 - 低沉嗡鸣 */
function playGravitySound(mode, strength) {
  ensureAudio();
  const ac = audioCtx, now = ac.currentTime;
  const isAttract = mode === 'attract';

  // 主音：低频持续嗡鸣
  const baseFreq = isAttract ? (strength * 40 + 60) : (strength * 50 + 80);
  const o = ac.createOscillator(), g = ac.createGain();
  o.connect(g); g.connect(ac.destination);
  o.type = 'sine';
  o.frequency.setValueAtTime(baseFreq, now);
  ...
  if (isAttract) {
    o.frequency.exponentialRampToValueAtTime(baseFreq * 1.5, now + 1.5);
    g.gain.setValueAtTime(0, now);
    g.gain.linearRampToValueAtTime(0.10, now + 0.3);
    g.gain.setValueAtTime(0.10, now + 1.2);
    g.gain.exponentialRampToValueAtTime(0.001, now + 1.5);
  } else {
    o.frequency.exponentialRampToValueAtTime(baseFreq * 0.6, now + 0.8);
    g.gain.setValueAtTime(0, now);
    g.gain.linearRampToValueAtTime(0.10, now + 0.1);
    g.gain.setValueAtTime(0.10, now + 0.6);
  ...
  if (isAttract) {
    o2.frequency.exponentialRampToValueAtTime(baseFreq * 3, now + 1.5);
  } else {
    o2.frequency.exponentialRampToValueAtTime(baseFreq * 1.3, now + 0.8);
  }
  g2.gain.setValueAtTime(0, now);
  g2.gain.linearRampToValueAtTime(0.04, now + 0.2);
  g2.gain.exponentialRampToValueAtTime(0.001, now + (isAttract ? 1.4 : 0.7));
  o2.start(now); o2.stop(now + 1.6);
}

/** 宝石连锁触发音效 - 多层水晶共鸣 */
function playGemChainSound(layer) {
  ensureAudio();
  const ac = audioCtx, now = ac.currentTime;

  const freqs = [1200, 900, 600];
  const freq = freqs[layer] || 800;
  const dur = layer === 0 ? 0.5 : 0.35;
  ...
  h.type = 'sine';
  h.frequency.setValueAtTime(freq * 1.5, now + 0.03);
  h.frequency.exponentialRampToValueAtTime(freq, now + dur * 0.8);
  hg.gain.setValueAtTime(0, now + 0.03);
  hg.gain.linearRampToValueAtTime(0.10, now + 0.06);
  hg.gain.exponentialRampToValueAtTime(0.001, now + dur * 0.9);
  h.start(now + 0.03); h.stop(now + dur + 0.1);

  // 高频闪烁
  const sparkle = ac.createBufferSource();
  sparkle.buffer = createNoiseBuffer(0.1);
  const sf = ac.createBiquadFilter(), sg = ac.createGain();
  sparkle.connect(sf); sf.connect(sg); sg.connect(ac.destination);
  sf.type = 'highpass';
  sf.frequency.value = 5000;
